<section class="space-y-6">
    ...
    <x-button variant="destructive" x-data=""
        x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion')">{{ __('Delete Account') }}</x-button>

    <x-modal maxWidth="lg" name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}">
            ...
            <div class="mt-6">
                ...
                <x-input id="password" name="password" type="password" class="block w-3/4 mt-1"
                    placeholder="{{ __('Password') }}" :messages="$errors->userDeletion->get('password')" />
            ...
            <div class="flex justify-end mt-6">
                ...
                <x-button class="ms-3" variant="destructive" type="submit">
                    {{ __('Delete Account') }}
                </x-button>
            </div>
        </form>
